...
async function main() {
  ...
  let stakingContract = await StakingContractFactory.attach(SXNODE_CONTRACT_ADDRESS) as Staking;
  
  for (let i = 1; i < signers.length; i++) {
    console.log(`Stake: address=${SXNODE_CONTRACT_ADDRESS}, account=${signers[i].address}`);
